...
namespace DTcms.DAL
{
    ...
    public partial class article
    {
        ...
        /// 获得前几行数据
        /// </summary>
        public DataSet GetList(int Top, string strWhere, string filedOrder,bool isLawView)
        {
            if (!isLawView)
            ...
            else 
            {
                ...
                strSql.Append(" id,update_time,real_name,approval_content,file_ext,title,category,file_path ");
                strSql.Append(" FROM view_law_manager ");
                if (strWhere.Trim() != "")
                {
                    strSql.Append(" where " + strWhere);
                }
        ...
        /// <param name="name">附表字段名</param>
        /// <param name="value">附表字段值</param>
        /// <returns></returns>
        public DataSet GetList(int Top, string strWhere, string name,string value, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (Top > 0)
            ...
            strSql.Append(" main.id,main.channel_id,main.category_id,main.call_index,main.title,main.link_url,main.img_url,main.seo_title,main.seo_keywords,main.seo_description,main.zhaiyao,main.content,main.sort_id,main.click,main.status,main.groupids_view,main.vote_id,main.is_top,main.is_red,main.is_hot,main.is_slide,main.is_sys,main.is_msg,main.user_name,main.add_time,main.update_time,minor."+ name + " ");
            strSql.Append(" FROM " + databaseprefix + "article main, " + databaseprefix + "article_attribute_value minor");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where main.id=minor.article_id AND main." + strWhere +" AND minor." + name +" like '%" + value + "%'");
            }
            strSql.Append(" order by " + filedOrder);
            return DbHelperSQL.Query(strSql.ToString());
        }
        ...
        /// <param name="value">附表字段值</param>
        /// <returns></returns>
        public DataSet GetList2(int Top, string strWhere, string name, string value, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (Top > 0)
            ...
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where main.id=minor.article_id AND main." + strWhere + " AND minor." + name + " = '" + value + "'");
            }
            strSql.Append(" order by " + filedOrder);
            return DbHelperSQL.Query(strSql.ToString());
        }
