...
namespace DTcms.DAL
{
    ...
    public partial class article_attribute_field
    {
        ...
        public int Add(Model.article_attribute_field model)
        {
            ...
            strSql.Append("name,title,control_type,data_type,data_length,data_place,item_option,default_value,is_required,is_password,is_html,editor_type,valid_tip_msg,valid_error_msg,valid_pattern,sort_id,is_sys,is_classify)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@is_classify", SqlDbType.TinyInt,1),
            ...
            parameters[17].Value = model.is_classify;
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("is_classify=@is_classify");
                        strSql.Append(" where id=@id");
                        SqlParameter[] parameters = {
					            new SqlParameter("@id", SqlDbType.Int,4),
					            new SqlParameter("@name", SqlDbType.NVarChar,100),
                        ...
                        parameters[18].Value = model.is_classify;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,name,title,control_type,data_type,data_length,data_place,item_option,default_value,is_required,is_password,is_html,editor_type,valid_tip_msg,valid_error_msg,valid_pattern,sort_id,is_sys,is_classify");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["is_classify"].ToString() != "")
                {
                    model.is_classify = int.Parse(ds.Tables[0].Rows[0]["is_classify"].ToString());
                }
                return model;
            }
            else
        ...
        /// 获得前几行频道对应的数据
        /// </summary>
        public DataSet GetList(int top,int channel_id, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + top.ToString());
            strSql.Append(" dt_article_attribute_field.* ");
            strSql.Append(" FROM " + databaseprefix + "article_attribute_field INNER JOIN " + databaseprefix + "channel_field ON " + databaseprefix + "article_attribute_field.id = " + databaseprefix + "channel_field.field_id");
            strSql.Append(" where channel_id=" + channel_id);
            if (strWhere.Trim() != "")
            {
        ...
        /// <summary>
        /// 获得begin行——end行频道对应的数据
        /// </summary>
        public DataSet GetList(int begin,int end,int channel_id, string strWhere)
        {
            //string sql = "select top (end-(begin-1)) * from table where ";
            //    Select top (n-(m-1)) * from [tablename] where [parimary key] 
            //not in(select top (m-1) [parimary key]
            //from [tablename] order by [排序字段及排序方法]) order by [排序字段及排序方法];

            //select top sum * from 表名 where
            //id not in (select top sum *(num -1) id from 表名 where 字段名 > 
            //“变量” and .... order by ** desc） 
            //and 字段名 > “变量” and .... order by ** desc
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + (end - (begin - 1)).ToString());
            strSql.Append(" dt_article_attribute_field.* ");
            strSql.Append(" FROM " + databaseprefix + "article_attribute_field INNER JOIN " + databaseprefix + "channel_field ON " + databaseprefix + "article_attribute_field.id = " + databaseprefix + "channel_field.field_id");
            strSql.Append(" where channel_id=" + channel_id);
            strSql.Append(" and dt_article_attribute_field.id not in(select top " + (begin - 1).ToString() + " dt_article_attribute_field.id ");
            strSql.Append(" FROM " + databaseprefix + "article_attribute_field INNER JOIN " + databaseprefix + "channel_field ON " + databaseprefix + "article_attribute_field.id = " + databaseprefix + "channel_field.field_id");
            strSql.Append(" where channel_id=" + channel_id);
            if (strWhere.Trim() != "")
            {
