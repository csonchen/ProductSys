...
namespace DTcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
            ...
            foreach (Model.article_attribute_field modelt in ls)
            {
                ...
                switch (modelt.control_type)
                {
                    case "single-text": //单行文本
                        ...
                        //设置默认值
                        //如果是修改者，则显示不可修改且默认为登陆者姓名
                        if (modelt.name == "modify_person")
                        {
                            txtControl.Enabled = false;
                            txtControl.Text = GetAdminInfo().real_name;
                        }
                        else
                    ...
                    case "datetime"://日期时间
                        break;
                    case "checkbox": //复选框
                        System.Web.UI.WebControls.CheckBox cbControl = new System.Web.UI.WebControls.CheckBox();
        ...
        private void TreeBind(int _channel_id)
        {
            ...
            System.Data.DataTable dt = bll.GetList(0, _channel_id);
                ...
                switch (modelt1.control_type)
                {
                    case "single-text": //单行文本
                        ...
                        if (txtControl != null && model.fields.ContainsKey(modelt1.name))
                        {
                            ...
                            else if (modelt1.name == "modify_person")//如果扩展字段为修改者，则显示姓名
                            { 
                                BLL.manager m = new BLL.manager();
                                txtControl.Text = m.GetModel(int.Parse(model.fields[modelt1.name])).real_name;
                            }
                            else
                            {
                    ...
                    case "number": //数字
                        goto case "single-text";
                    case "checkbox": //复选框
                        System.Web.UI.WebControls.CheckBox cbControl = FindControl("field_control_" + modelt1.name) as System.Web.UI.WebControls.CheckBox;
        ...
        {
            System.Data.DataTable dt = new BLL.article_attribute_field().GetList(_channel_id, "").Tables[0];
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                switch (dr["control_type"].ToString())
                {
                    case "single-text": //单行文本
                        ...
                        {
                            //如果扩展字段是修改者，则保留ID
                            if (dr["name"].ToString() == "modify_person")
                            {
                                dic.Add(dr["name"].ToString(), GetAdminInfo().id.ToString());
                            }else
                    ...
                    case "number": //数字
                        goto case "single-text";
                    case "checkbox": //复选框
                        System.Web.UI.WebControls.CheckBox cbControl = FindControl("field_control_" + dr["name"].ToString()) as System.Web.UI.WebControls.CheckBox;
                        if (cbControl != null)
                        {
        ...
        private bool DoAdd()
        {
                ...
                && attachFileNameArr.Length > 0 && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0 && attachPointArr.Length > 0)
            {
                ...
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                    ...
                    //创建正则匹配文档后缀  
                    Regex docRegex = new Regex("^(doc|docx|wps|txt)$");
                    Regex excelRegex = new Regex("^(xls|xlsx)$");
                    //保存文档后缀
                    string fileSuffix = attachFilePathArr[i].Split('.')[1];
                    //正则匹配
                    Match docMatch = docRegex.Match(fileSuffix);
                    Match excelMatch = excelRegex.Match(fileSuffix);

                    //文档文件允许转换
                    if (docMatch.Success)
                    {
                        DocToHtml(attachFilePathArr[i]);
                    }
                    else if (excelMatch.Success) //excel文件格式转化
                    {
                        ExcelToHtml(attachFilePathArr[i]);
                    }   
                 }
                model.attach = ls;
            }
        ...
        //wps文档转为html格式(上传到当前目录下)
        public void DocToHtml(string fullFilePath)
        {
            ...
            //打开上传文件
            object fileName = Server.MapPath(fullFilePath);
            WPS.Document doc = wps.Documents.Open(fileName.ToString());

            //上传对应的html文件
            string htmlPath = fullFilePath.Split('.')[0];
            htmlPath += ".html";

            string[] strs = htmlPath.Split('/');
            string dirPath = strs[0] + "/" + strs[1] + "/" + strs[2] + "/" + strs[3];
            //判断文件夹是否存在, 不存在则创建
            ...
            object savePath = htmlPath; //"f:\\aaa.html";

            //string str = wps.FileConverters.Item(0).FormatName;
            doc.SaveAs(savePath.ToString(), WPS.WdSaveFormat.wdFormatHTML);
        ...
        //excel转为html格式（上传到当前目录下）
        public void ExcelToHtml(string fullFilePath)
        {
            ...
            try
            {
                //打开excel文件
                object fileName = Server.MapPath(fullFilePath);
                excel.Workbooks.Open(Filename: fileName.ToString());
                excel.Visible = false;
                ...
                if (excel.Workbooks.Count > 0)
            {
                    ...
                    while (wsEnumerator.MoveNext())
                {
                        ...
                        wsCurrent.SaveAs(Filename: htmlPath, FileFormat: format);
                        ++i;
                        break;
                }
